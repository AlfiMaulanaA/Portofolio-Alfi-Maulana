...
export function useMqttPalm() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      client.on("connect", () => {
        console.log("✅ MQTT Connected to palm device successfully");
        ...
        client.subscribe("palm/status", { qos: 1 }, (err) => {
          if (err) {
            ...
            console.log("📡 Subscribed to palm/status topic");
            setState((prev) => ({
              ...prev,
              isConnected: true,
              isConnecting: false,
  ...
  const handlePalmStatusMessage = useCallback((messageStr: string) => {
    try {
      ...
      console.log("📊 Palm status message received:", message);
  ...
  const startRegistration = useCallback(
    async (userName: string) => {
      ...
      try {
        ...
        console.log("📤 Sending registration command:", command);

        // Publish registration command to palm/control topic
        clientRef.current!.publish(
          ...
          (error) => {
            if (error) {
              ...
              console.log("✅ Registration command sent successfully");
  ...
  const reset = useCallback(() => {
    ...
    console.log("🔄 Palm registration state reset");
  }, []);
