...
export function useMqttPalmRecognition() {
  ...
  const fetchTodayStats = useCallback(async () => {
    try {
      ...
      if (result.success) {
        ...
        console.log("✅ Today's stats loaded:", result.data);
      } else {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `🔌 Connecting to MQTT broker for palm recognition: ${brokerUrl}`
      );
      ...
      client.on("connect", () => {
        console.log("✅ MQTT Connected for palm recognition");
        ...
        client.subscribe("palm/compare/result", { qos: 1 }, (err) => {
          if (err) {
            ...
            console.log("📡 Subscribed to palm/compare/result topic");
            setState((prev) => ({
              ...prev,
              isConnected: true,
              isConnecting: false,
  ...
  const handlePalmRecognitionResult = useCallback(
    async (messageStr: string) => {
      try {
        ...
        console.log("📊 Palm recognition result received:", result);
        ...
        try {
          ...
          console.log("✅ History log created for palm recognition");
        } catch (error) {
  ...
  useEffect(() => {
    console.log("🚀 Initializing MQTT Palm Recognition connection...");
    fetchTodayStats();
    connect();

    return () => {
      console.log("🧹 Cleaning up MQTT Palm Recognition connection...");
      disconnect();
    };
  }, []);
