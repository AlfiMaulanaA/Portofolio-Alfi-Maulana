...
export function useMqttAttendance() {
  ...
  const getAttendanceType = (type: number): { name: string; icon: string; color: string } => {
    switch (type) {
      case 1:
  ...
  const showAttendanceNotification = useCallback((payload: AttendancePayload) => {
  ...
  const publishAttendance = useCallback((userId: string, type: number, userName?: string) => {
    ...
    try {
      clientRef.current.publish("acs_front_attendance", JSON.stringify(payload), { qos: 1 })
      console.log(`✅ Published attendance for User ${userId} (Type: ${type})`)
      return true
    } catch (error) {
    ...
    try {
      ...
      client.on("connect", () => {
        ...
        client.subscribe("acs_front_attendance", { qos: 1 }, (err) => {
          if (err) {
            console.error("❌ Failed to subscribe to acs_front_attendance:", err)
            setState((prev) => ({
              ...prev,
